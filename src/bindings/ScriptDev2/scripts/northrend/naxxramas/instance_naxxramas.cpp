        ...
        switch(pCreature->GetEntry())
        {
			...
			case NPC_GLUTH:
				if (!pCreature->isAlive())
					SetData(TYPE_GLUTH, DONE);
				break;
			case NPC_GOTHIK:
				if (!pCreature->isAlive())
					SetData(TYPE_GOTHIK, DONE);
				break;
			case NPC_GROBBULUS:
				if (!pCreature->isAlive())
					SetData(TYPE_GROBBULUS, DONE);
				break;
			case NPC_HEIGAN:
				if (!pCreature->isAlive())
					SetData(TYPE_HEIGAN, DONE);
				break;
			case NPC_LOATHEB:
				if (!pCreature->isAlive())
					SetData(TYPE_LOATHEB, DONE);
				break;
			case NPC_MAEXXNA:
				if (!pCreature->isAlive())
					SetData(TYPE_MAEXXNA, DONE);
				break;
			case NPC_NOTH:
				if (!pCreature->isAlive())
					SetData(TYPE_NOTH, DONE);
				break;
			case NPC_PATCHWERK:
				if (!pCreature->isAlive())
					SetData(TYPE_PATCHWERK, DONE);
				break;
			case NPC_RAZUVIOUS:
				if (!pCreature->isAlive())
					SetData(TYPE_RAZUVIOUS, DONE);
				break;
            case NPC_ANUB_REKHAN:
				if (!pCreature->isAlive())
					SetData(TYPE_ANUB_REKHAN, DONE);
                m_uiAnubRekhanGUID = pCreature->GetGUID();
                break;
            case NPC_FAERLINA:
				if (!pCreature->isAlive())
					SetData(TYPE_FAERLINA, DONE);
                m_uiFaerlinanGUID = pCreature->GetGUID();
                break;
            case NPC_THADDIUS:
				if (!pCreature->isAlive())
					SetData(TYPE_THADDIUS, DONE);
