...
struct MANGOS_DLL_DECL boss_ymironAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        m_uiAncestors_Vengeance_Timer = (m_bIsHeroicMode ? 60000 : 45000);
        m_uiPause_Timer               = 0;
        ...
        m_uiHealthAmountMultipler = (m_bIsHeroicMode ? 20 : 25);
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (!m_bIsPause)
        {
            ...
            {
                DoCast(m_creature, m_bIsHeroicMode ? SPELL_BANE_H : SPELL_BANE_N);
                m_uiBane_Timer = 20000+rand()%5000;
            } else m_uiBane_Timer -= uiDiff;

            if (m_uiFetidRot_Timer < uiDiff)
            {
                DoCast(m_creature->getVictim(), m_bIsHeroicMode ? SPELL_FETID_ROT_H : SPELL_FETID_ROT_N);
            ...
            if (m_uiAncestors_Vengeance_Timer < uiDiff)
            {
                ...
                m_uiAncestors_Vengeance_Timer = (m_bIsHeroicMode ? 60000+rand()%5000 : 45000+rand()%5000);
            ...
            if (m_bIsActiveWithBJORN && m_uiAbility_BJORN_Timer < uiDiff)
            {
                ...
                if (Creature* pTemp = m_creature->SummonCreature(NPC_SPIRIT_FOUNT, 385+rand()%10, -330+rand()%10, 104.756, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 1000))
                {
					pSpirit = pTemp;
                    pTemp->SetSpeed(MOVE_RUN, 0.4f);
                    pTemp->CastSpell(pTemp, m_bIsHeroicMode ? SPELL_SPIRIT_FOUNT_H : SPELL_SPIRIT_FOUNT_N, true);
                    pTemp->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                    pTemp->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
            ...
            {
                DoCast(m_creature->getVictim(), m_bIsHeroicMode ? SPELL_SPIRIT_STRIKE_H : SPELL_SPIRIT_STRIKE_N);
            ...
            {
                DoCast(m_creature, m_bIsHeroicMode ? SPELL_SPIRIT_BURST_H : SPELL_SPIRIT_BURST_N);
            ...
            if ((m_creature->GetHealth()*100 / m_creature->GetMaxHealth()) < (100-(m_uiHealthAmountMultipler * m_uiHealthAmountModifier)))
            {
                ...
                m_creature->SendMonsterMove(ActiveBot[m_uiActiveOrder[m_uiOrder]].MoveX, ActiveBot[m_uiActiveOrder[m_uiOrder]].MoveY, ActiveBot[m_uiActiveOrder[m_uiOrder]].MoveZ, 0, m_creature->GetMonsterMoveFlags(), 1);
