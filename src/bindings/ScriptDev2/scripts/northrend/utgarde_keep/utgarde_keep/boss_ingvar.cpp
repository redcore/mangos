...
struct MANGOS_DLL_DECL npc_annhylde_the_callerAI : public ScriptedAI
{
    npc_annhylde_the_callerAI(Creature *pCreature) : ScriptedAI(pCreature)
    {
        pInstance =(ScriptedInstance*)pCreature->GetInstanceData();
		Reset();
    }
    ...
    void Reset()
    {
        m_creature->addUnitState(MOVEMENTFLAG_FLYING + MOVEMENTFLAG_UNK3);
        ...
        m_creature->Relocate(x+1,y,z+30);
		m_creature->SendMonsterMove(x+1,y,z+30,0,MONSTER_MOVE_NONE,0);

        Unit* ingvar = Unit::GetUnit(*m_creature, pInstance ? pInstance->GetData64(NPC_INGVAR) : 0);
        if (ingvar)
        {
...
//            DoScriptText(YELL_RESSURECT,m_creature);
    ...
    void MovementInform(uint32 type, uint32 id)
    {
        if (type != POINT_MOTION_TYPE)
            return;
        Unit* ingvar = pInstance->instance->GetCreature(pInstance->GetData64(NPC_INGVAR));
        if (ingvar)
        {
            switch (id)
            {
            case 1:
                ...
                DoCast(ingvar,SPELL_SCOURG_RESURRECTION_BEAM,true);
                Resurect_Timer = 8000;
                Resurect_Phase = 1;
                break;
            case 2:
    ...
    void UpdateAI(const uint32 diff)
    {
        if (Resurect_Timer)
            if (Resurect_Timer <= diff)
            {
                ...
                {
                    Creature* ingvar = pInstance->instance->GetCreature(pInstance->GetData64(NPC_INGVAR));
                    if (ingvar)
                    {
...
{
    return new npc_annhylde_the_callerAI (pCreature);
}
...
struct MANGOS_DLL_DECL mob_ingvar_throw_dummyAI : public ScriptedAI
{
    ...
    void Reset()
    {
        Unit *pTarget = pInstance->instance->GetCreature(pInstance->GetData64(NPC_AXE_TARGET));
        if (pTarget)
        {
